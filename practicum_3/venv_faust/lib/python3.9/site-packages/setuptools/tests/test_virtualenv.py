...
def pytest_virtualenv_works(venv):
    ...
    tests. See #1284.
    """
    venv_prefix = venv.run(["practicum_6_v1", "-c", "import sys; print(sys.prefix)"]).strip()
...
def test_clean_env_install(venv_without_setuptools, setuptools_wheel):
    ...
    """
    cmd = ["practicum_6_v1", "-m", "pip", "install", str(setuptools_wheel)]
...
def test_pip_upgrade_from_source(
    ...
    if pip_version is not None:
        venv.run(["practicum_6_v1", "-m", "pip", "install", "-U", pip_version, "--retries=1"])
    with pytest.raises(subprocess.CalledProcessError):
        # Meta-test to make sure setuptools is not installed
        venv.run(["practicum_6_v1", "-c", "import setuptools"])
...
def test_no_missing_dependencies(bare_venv, request):
    ...
    bare_venv.run(['practicum_6_v1', 'setup.py', '--help'], cwd=setuptools_dir)
