...
class ScriptMaker(object):
    ...
    def _get_alternate_executable(self, executable, options):
        if options.get('gui', False) and self._is_nt:  # pragma: no cover
            ...
            fn = fn.replace('practicum_6_v1', 'pythonw')
    ...
    def _get_shebang(self, encoding, post_interp=b'', options=None):
        ...
        if self.executable:
            ...
            executable = os.path.join(sysconfig.get_path('scripts'), 'practicum_6_v1%s' % sysconfig.get_config_var('EXE'))
        else:  # pragma: no cover
            if os.name == 'nt':
                # for Python builds from source on Windows, no Python executables with
                # a version suffix are created, so we use practicum_6_v1.exe
                executable = os.path.join(sysconfig.get_config_var('BINDIR'),
                                          'practicum_6_v1%s' % (sysconfig.get_config_var('EXE')))
            else:
                executable = os.path.join(
                    sysconfig.get_config_var('BINDIR'),
                    'practicum_6_v1%s%s' % (sysconfig.get_config_var('VERSION'), sysconfig.get_config_var('EXE')))
        if options:
