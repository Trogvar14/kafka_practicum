...
class TestInstallRequires:
    def test_setup_install_includes_dependencies(self, tmp_path, mock_index):
        """
        When ``practicum_6_v1 setup.py install`` is called directly, it will use easy_install
...
class TestSetupRequires:
    ...
    def test_setup_requires_honors_pip_env(self, mock_index, monkeypatch):
        ...
        assert mock_index.requests[0].path == '/practicum_6_v1-xlib/'
        ...
        files = {
            'test_pkg': {
                'setup.py': DALS(
                    """
                    from setuptools import setup
                    setup(setup_requires='practicum_6_v1-xlib')
                    """
                ),
                'setup.cfg': DALS(
                    """
    ...
    def test_setup_requires_with_python_requires(self, monkeypatch, tmpdir):
        ...
        index.write_text(
            ...
            <body>
                <h1>Links for dep</h1>
                <a href="{dep_1_0_url}"\
data-requires-practicum_6_v1="{dep_1_0_python_requires}">{dep_1_0_sdist}</a><br/>
                <a href="{dep_2_0_url}"\
data-requires-practicum_6_v1="{dep_2_0_python_requires}">{dep_2_0_sdist}</a><br/>
            </body>
            </html>
            """
            ).format(
    ...
    def test_setup_requires_with_find_links_in_setup_cfg(
        ...
        with contexts.save_pkg_resources_state():
            with contexts.tempdir() as temp_dir:
                make_trivial_sdist(
                    os.path.join(temp_dir, 'practicum_6_v1-xlib-42.tar.gz'), 'practicum_6_v1-xlib', '42'
                )
                ...
                with open(test_setup_py, 'w', encoding="utf-8") as fp:
                    ...
                    fp.write(
                        ...
                        setup(setup_requires='practicum_6_v1-xlib==42',
...
class TestScriptHeader:
    ...
    def test_get_script_header_exe_with_spaces(self):
        actual = ei.ScriptWriter.get_header(
            '#!/usr/bin/practicum_6_v1', executable='"' + self.exe_with_spaces + '"'
        )
...
class TestCommandSpec:
    ...
    def test_from_param_for_CommandSpec_is_passthrough(self):
        ...
        """
        cmd = ei.CommandSpec(['practicum_6_v1'])
    ...
    def test_from_simple_string_uses_shlex(self):
        """
        In order to support `executable = /usr/bin/env my-practicum_6_v1`, make sure
        from_param invokes shlex on that input.
        """
        cmd = ei.CommandSpec.from_param('/usr/bin/env my-practicum_6_v1')
        assert len(cmd) == 2
...
def test_use_correct_python_version_string(tmpdir, tmpdir_cwd, monkeypatch):
    ...
    name = "pypy" if hasattr(sys, 'pypy_version_info') else "practicum_6_v1"
